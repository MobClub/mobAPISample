...
public class AddressToPostcodeActivity extends Activity implements APICallback, OnItemSelectedListener {
	...
	private void onDistrictListGot(Map<String, Object> result) {
		...
		for (HashMap<String, Object> p : resultList) {
			provinceIds.add(com.mob.tools.utils.R.toString(p.get("id")));
			provinces.add(com.mob.tools.utils.R.toString(p.get("province")));
		}
		...
		for (HashMap<String, Object> add : addressList) {
			...
			if (addList != null) {
				for (String a : addList) {
					addresses.add(a);
					addressToPostcode.put(a, com.mob.tools.utils.R.toString(add.get("PostNumber")));
				}
			}
		}
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
		if (parent.equals(spProvince)) {
			...
			for (HashMap<String, Object> c : cityList) {
				cityIds.add(Integer.parseInt(com.mob.tools.utils.R.toString(c.get("id"))));
			...
			for (HashMap<String, Object> d : districtList) {
				districtIds.add(Integer.parseInt(com.mob.tools.utils.R.toString(d.get("id"))));
